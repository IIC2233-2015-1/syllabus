...
class Caja:
    ...
    def __init__(self, t):
        ...
        self.termino = 2 * t
        """
        inicialmente el tiempo de término de atención debe estar fuera de rango 
        ya que no debe salir ningún cliente si es que nadie se está atendiendo.
...
class Simulacion:
    ...
    def sale_primero(self, si, no):
        ...
        print("[inicio] sale caja", si, len(
            self.banco.cajas[si].cola), len(self.banco.cajas[no].cola))
        if len(self.banco.cajas[si].cola) > 0:
            cola.popleft()
        if len(self.banco.cajas[no].cola) > len(self.banco.cajas[si].cola) + 1 >= 0:
            print("cambio a 0")
            self.banco.cajas[si].cola.append(
                self.banco.cajas[no].cola.popleft())
        if len(self.banco.cajas[si].cola) == 0:
            self.banco.cajas[si].termino = 2 * self.tiempo_maximo_sim
        else:
            self.banco.cajas[
                si].termino = self.tiempo_simulacion + uniform(1, self.salida)
        print("[final] sale caja", si, len(self.banco.cajas[si].cola),
              len(self.banco.cajas[no].cola))
    ...
    def llega_primero(self, primero):
        ...
        if len(self.banco.cajas[0].cola) <= len(self.banco.cajas[1].cola):
            if len(self.banco.cajas[0].cola) == 0:
                self.banco.cajas[
                    0].termino = self.tiempo_simulacion + uniform(1, self.salida)
